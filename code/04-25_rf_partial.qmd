...
weather_recipe <-
  ...
  recipe(strength_gtex ~ ., #change response variable name to categorical col name
         data = weather_train) %>%
...
First, let's specify:  
  - the **type of model** we want to train  (#name the model/algorithm)
  - which **engine** we want to use  (#what package are you using)
  - which **mode** we want to use  (#regression/classification)
...
rf_spec <- 
  ...
  set_engine("ranger") %>% 
    # Specifying mode  
  set_mode("regression")#"classification for categorical variable
rf_spec
```
...
rf_grid_result <- tune_sim_anneal(object = rf_spec,
                    ...
                    #param_info = rf_param,(donot have to do this, because we donot have hyperparameters in engine level)
                    iter = 10 #50 is thumbrule, but takes long time; it did well on 8 so we're going with 10.
                     )
...
best_rmse <- rf_grid_result %>% 
  select_by_pct_loss("trees",
                     metric = "rmse",
                     limit = 2 )#percent limit; willing to loose 2% of rmse; higher value-less strict
...
Based on R2, we would choose   (got different on my own processing)
  - mtry = 24
  - trees = 518
...
final_spec <- rand_forest(trees = best_r2$trees,
  ...
  # Specify the engine
  set_engine("ranger",
             importance = "permutation") %>% #could be used "impurity" instead as well. But permutation helps on assessing the differences in accuracy
    # Specifying mode  
  set_mode("regression")
...
```{r final_fit}
final_fit <- last_fit(final_spec, #train the model on whole training set at once
                weather_recipe,
                split = weather_split)
...
#our training model was able to fit the model quite well (95%), however on testing set, R2 was only 21%
